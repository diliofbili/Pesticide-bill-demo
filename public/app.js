...
function editItemQuantity(index) {
    ...
    // Simple regex to detect common quantity patterns (number + unit)
    const quantityRegex = /(\d+)\s*(gm|ml|kg|g|l|mg|tablets|tabs|pcs|pieces|capsules|caps)$/i;
    ...
    // Prompt with the original quantity if detected
    let promptMessage = `Enter new quantity for ${originalName}:`;
    if (originalQuantity && originalUnit) {
        promptMessage = `Enter new quantity for ${baseName} (current: ${originalQuantity} ${originalUnit}):`;
    }
    
    const newQuantityInput = prompt(promptMessage, originalQuantity);
...
// Handle search functionality - Fixed to show items when searching by salt composition
function handleSearch(event) {
    ...
    // IMPROVED: First identify salt categories that match the search term by either name OR composition
    const matchingSaltCategories = new Set();
    
    pesticideData.forEach(item => {
        if (item.price === 0) { // Salt category items have price 0
            const nameMatches = item.name.toLowerCase().includes(searchTerm);
            const compositionMatches = item.saltComposition && item.saltComposition.toLowerCase().includes(searchTerm);
            
            // Add to matching categories if either name or composition matches
            if (nameMatches || compositionMatches) {
                matchingSaltCategories.add(item.name);
            }
        }
    });
    
    // Filter items based on search term OR belonging to a matching salt category
    const filteredItems = pesticideData.filter((item, index) => {
        // Direct match on this item's properties
        const matchesSearch = 
        ...
        // Check if this item belongs to a matching salt category
        let belongsToMatchingSaltCategory = false;
        
        if (matchingSaltCategories.size > 0 && item.price > 0) {
            // Find the nearest preceding salt category for this item
            let saltCategoryName = null;
            
            for (let i = index - 1; i >= 0; i--) {
                if (pesticideData[i].price === 0) {
                    saltCategoryName = pesticideData[i].name;
                    break;
                }
            }
            
            // If this item's salt category is in our matching set, include the item
            if (saltCategoryName && matchingSaltCategories.has(saltCategoryName)) {
                belongsToMatchingSaltCategory = true;
            }
        }
        
        // IMPROVED: To ensure all salt categories and their items are shown,
        // if the item is a salt category that matches, always include it AND
        // always include the next items until another salt category is found
        if (item.price === 0 && matchingSaltCategories.has(item.name)) {
            return true;
        }
        
        // Include item if it directly matches the search OR belongs to a matching salt category
        return matchesSearch || belongsToMatchingSaltCategory;
    });
...
function editItemQuantity(index) {
    ...
    // Set the label message based on detected quantity
    let labelMessage = `Enter quantity for ${originalName}:`;
    if (originalQuantity && originalUnit) {
        labelMessage = `Current quantity is ${originalQuantity} ${originalUnit} for ${baseName}`;
    }
...
// Replace edit quantity functionality with custom modal
function editQuantity(index) {
    const itemIndex = selectedItems.findIndex(i => i.index === index);
    if (itemIndex !== -1) {
        const item = selectedItems[itemIndex];
